import type { ListDimensionValuesParams, ListDimensionValuesResponse } from '../types.js'

export const listDimensionValuesTool = {
...
export async function listDimensionValues(
  apiKey: string,
  baseUrl: string,
  params: ListDimensionValuesParams
): Promise<ListDimensionValuesResponse> {
  try {
    const url = new URL('/data/v1/dimensions/{DIMENSION_ID}', baseUrl)
    
    // Replace path parameter DIMENSION_ID
    url.pathname = url.pathname.replace('DIMENSION_ID', encodeURIComponent(String(params.DIMENSION_ID)))
