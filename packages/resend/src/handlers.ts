...
export class ResendHandlers {
  ...
  async sendEmail(params: SendEmailParams): Promise<EmailResponse> {
    try {
      ...
      const emailParams = {
        ...
        html:
          params.html ||
          (params.text ? undefined : "<p>No content provided</p>"),
      };
  ...
  async sendBatchEmails(
    ...
    try {
      ...
      const emailsWithContent = params.emails.map((email) => ({
        ...email,
        html:
          email.html || (email.text ? undefined : "<p>No content provided</p>"),
      }));
      const result = await this.resend.batch.send(emailsWithContent as any);
      return {
        data: Array.isArray(result.data)
          ? result.data.map((item: any) => ({ id: item.id }))
          : [],
      };
    } catch (error) {
      throw new Error(
  ...
  async updateEmail(params: UpdateEmailParams): Promise<EmailResponse> {
    try {
      ...
      const result = (await (this.resend.emails as any).update?.(
        id,
        updateData
      )) || { data: { id } };
